...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const aboutSwiper = new Swiper('.about-slider', {
      ...
      slidesPerView: 1,
      speed: 800,
       // Выключаем встроенную обработку мыши, сделаем её вручную для 100% контроля
...
sliderEl.addEventListener('wheel', (e) => {
    ...
    const isFirst = aboutSwiper.isBeginning;
    const isLast = aboutSwiper.isEnd;

    // Если мы на последнем слайде и крутим ВНИЗ
    if (isLast && delta > 0) {
        // Ничего не делаем, даем событию проплыть к body
        return;
    }
    
    // Если мы на первом слайде и крутим ВВЕРХ
    if (isFirst && delta < 0) {
        return;
    }

    // Во всех остальных случаях (внутри слайдера) — 
    // не даем странице скроллиться, пусть работает Swiper
    e.stopPropagation();
}, { passive: false });


  const menuItems = document.querySelectorAll('.about-item');
  // Клик по меню переключает слайдер
